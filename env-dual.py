...
class TrafficEnv:
    def __init__(self, config_file, mode='binary', tripinfo_file="results/tripinfo.xml", seed="156"):
        ...
        self.sumoCmd = [
            ...
            '--scale', '1', '--tripinfo-output', self.tripinfo_file
        ]
        self.time = 0
    ...
    def get_reward(self):
        ...
        for i, intersection_ID in enumerate(traci.trafficlight.getIDList()):
            ...
            # Emergency rewards
            emergency_waiting_penalty = -2.0 * emergency_waiting
            max_speed = 60.0
            emergency_speed_reward = 2.0 * emergency_speed if emergency_count > 0 else 0
            ...
            total_emergency_reward = emergency_waiting_penalty + emergency_speed_reward + phase_switch_penalty
            ...
            total_emergency_reward = np.clip(total_emergency_reward, -200, 50)

            reward_normal.append(total_reward)
            reward_emergency.append(total_emergency_reward)
        return np.array(reward_normal), np.array(reward_emergency)

...
class TrafficEnv:
    def __init__(self, config_file, mode='binary', tripinfo_file="results/tripinfo.xml", seed="156"):
        ...
        self.sumoCmd = [self.sumoBinary, "-c", self.config_file, '--random', '1.5', '--no-step-log', '-W', '--seed', seed,
                        '--tripinfo-output',
                        self.tripinfo_file]
        self.time = 0
    ...
    def reset(self):
        traci1.start(self.sumoCmd)
        traci.start(self.sumoCmd, label='123')
        ...
        #
        #     # Get phase counts
            logic = traci1.trafficlight.getCompleteRedYellowGreenDefinition(intersection_ID)[0]
    ...
    # ... (rest of the methods remain unchanged, unless you want to include them)
        ...
        for lane_id in traci.trafficlight.getControlledLanes(intersection_ID):
            ...
            for vehicle_id in vehicle_list:
                if traci.vehicle.getTypeID(vehicle_id) == "emergency":
                    ...
                    emergency_speed += traci.vehicle.getSpeed(vehicle_id)
    ...
    #         emergency_waiting_penalty = -50000 * emergency_waiting
    #         max_speed = 60.0
    #         emergency_speed_penalty = -20000 * (max_speed - emergency_speed) if emergency_count > 0 else 0
    #         queue_reward = 0.5 * queue_change
    #         waiting_time_reward = 0.01 * waiting_time_change
    #         queue_penalty = -0.2 * current_queue
    #         total_reward = queue_reward + waiting_time_reward + queue_penalty + emergency_waiting_penalty + emergency_speed_penalty
    #
    ...
    def get_reward(self):
        ...
        for i, intersection_ID in enumerate(traci.trafficlight.getIDList()):
            ...
            queue_reward = 0.5 * queue_change
            waiting_time_reward = 0.01 * waiting_time_change
            speed_reward = 0.2 * current_speed  
            queue_penalty = -0.2 * current_queue
            phase_switches = sum(1 for lane in traci.trafficlight.getControlledLanes(intersection_ID) 
                                if traci.lane.getLastStepHaltingNumber(lane) > 0)
            phase_switch_penalty = -0.05 * phase_switches

            emergency_count, emergency_waiting, emergency_speed = self._get_emergency_metrics(intersection_ID)
            emergency_waiting_change = self.prev_emergency_waiting[intersection_ID] - emergency_waiting
            self.prev_emergency_waiting[intersection_ID] = emergency_waiting

            emergency_priority_factor = 1 if emergency_count > 0 else 0 
            emergency_waiting_penalty = -10.0 * emergency_waiting * emergency_priority_factor if emergency_count > 0 else 0
            emergency_speed_reward = 4.0 * emergency_speed * emergency_priority_factor


            # Total reward: Combine with emergency priority
            total_reward = (queue_reward +
                            waiting_time_reward +
                            speed_reward +
                            queue_penalty +
                            phase_switch_penalty +
                            emergency_waiting_penalty +
                            emergency_speed_reward)

            # Store rewards
            reward_normal.append(total_reward)
            reward_emergency.append(emergency_waiting_penalty + emergency_speed_reward)

        return reward_normal, reward_emergency
    '''
